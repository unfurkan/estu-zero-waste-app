...
class _ReviewRideState extends State<ReviewRide> {
  ...
  _initialiseDirectionsResponse() {
    ...
    int minutes = (widget.modifiedResponse['duration'] / 60).round();
    int seconds = (widget.modifiedResponse['duration'] % 60).round();
    duration = minutes.toString() + 'dk' + " " + seconds.toString() + "s";
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Rota Önizlemesi'),
      ),
      body: SafeArea(
        child: Stack(
          children: [
            ...
            reviewRideBottomSheet(context, distance, dropOffTime, duration),
          ],
        ),
      ),
    );
